...
ERROR_IF_UPTODATE=0
DIST_SRC="github"
INSTALL_DEST="/config/user-data/bin/v2ray"
CONFIG_DIR="/config/user-data/etc/v2ray"
LOG_DIR="/config/user-data/var/log/v2ray"
INITSCRIPT_DIR_SRC="/config/user-data/etc/systemd/system"
INITSCRIPT_DIR_DEST="/etc/systemd/system"
...
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        VER=`${INSTALL_DEST}/v2ray -version 2>/dev/null`
        RETVAL="$?"
...
copyFile() {
    NAME=$1
    ERROR=`cp "${VSRC_ROOT}/${NAME}" "${INSTALL_DEST}/${NAME}" 2>&1`
    if [[ $? -ne 0 ]]; then
...
makeExecutable() {
    chmod +x "${INSTALL_DEST}/$1"
}

installV2Ray(){
    # Install V2Ray binary to ${INSTALL_DEST}
    ...
    if [[ ! -f "${CONFIG_DIR}/config.json" ]]; then
        mkdir -p ${CONFIG_DIR}
        mkdir -p ${LOG_DIR}
        cp "${VSRC_ROOT}/vpoint_vmess_freedom.json" "${CONFIG_DIR}/config.json"
        if [[ $? -ne 0 ]]; then
...
installInitScript(){
    if [[ -n "${SYSTEMCTL_CMD}" ]];then
        if [[ ! -f "${INITSCRIPT_DIR_SRC}/v2ray.service" ]]; then
                cp "${VSRC_ROOT}/systemd/v2ray.service" "${INITSCRIPT_DIR_SRC}/"
